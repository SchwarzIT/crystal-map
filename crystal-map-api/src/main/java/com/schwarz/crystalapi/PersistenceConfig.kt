        ...
        get() {
            if (mConnector == null) {
                throw RuntimeException(
                    "no database connector configured.. call PersistenceConfig.configure(Connector)",
                )
            }
            return mConnector!!
        }
    ...
    val suspendingConnector: SuspendingConnector
        get() {
            if (mSuspendingConnector == null) {
                throw RuntimeException(
                    "no database suspendingConnector configured.. call PersistenceConfig.configure(SuspendingConnector)",
                )
            }
