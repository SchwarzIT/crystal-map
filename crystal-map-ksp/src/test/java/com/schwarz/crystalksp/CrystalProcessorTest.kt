...
class CrystalProcessorTest {
    ...
    fun testTypeConverterGeneration() {
        ...
        Assert.assertEquals(KotlinCompilation.ExitCode.OK, compilation.exitCode)
        val actual = compilation.sourcesGeneratedBySymbolProcessor.find { it.name == "DateTypeConverterInstance.kt" }
            ?.readLines()
        Assert.assertEquals(expected, actual)
    }
    ...
    fun testTypeConverterExporterGeneration() {
        ...
        Assert.assertEquals(KotlinCompilation.ExitCode.OK, compilation.exitCode)
        val actual = compilation.sourcesGeneratedBySymbolProcessor.find { it.name == "TestTypeConvertersInstance.kt" }
    ...
    fun testTypeConverterExporterGenerationWithGenericTypes() {
        ...
        Assert.assertEquals(KotlinCompilation.ExitCode.OK, compilation.exitCode)
        val actual = compilation.sourcesGeneratedBySymbolProcessor.find { it.name == "TestTypeConvertersGenericInstance.kt" }
            ?.readLines()?.map { it.trim() }
    ...
    private fun compileKotlin(vararg sourceFiles: SourceFile, useSuspend: Boolean = false): JvmCompilationResult {
        return KotlinCompilation().apply {
            ...
            jvmTarget = "17"
            kspArgs["useSuspend"] = useSuspend.toString()
