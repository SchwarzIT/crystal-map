...
object ProcessingContext {
    ...
    fun KSTypeReference.resolveTypeNameWithProcessingTypes(): TypeName =
        this.resolve().resolveTypeNameWithProcessingTypes()
...
//    }
    ...
    data class DeclaringName(
        ...
        val realTypeName =
            ksType?.resolveTypeNameWithProcessingTypes()
                ?: (declaration as KSTypeParameter).toNullableSafeTypeVariableName()
        ...
        override fun asTypeName(): TypeName? {
            val className =
                createdQualifiedClassNames.firstOrNull {
            ...
            return when (declaration) {
                ...
                is KSTypeReference ->
                    declaration.resolveTypeNameWithProcessingTypes().copy(
                        isNullable(),
                    )
                else -> {
                    logger.error(
            ...
            declaration is KSTypeParameter ||
                (
                    declaration is KSPropertyDeclaration &&
                        declaration.type.resolve().declaration is KSTypeParameter
                )

        override fun isNullable(): Boolean =
            realTypeName.isNullable || nullableIndexes.contains(relevantIndex)

        override fun isProcessingType(): Boolean =
            createdQualifiedClassNames.any { it.canonicalName == name } &&
                (
                    name
                        .endsWith("Wrapper") ||
                        name
