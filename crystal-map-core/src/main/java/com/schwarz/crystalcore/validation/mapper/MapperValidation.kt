...
class MapperValidation<T>(
    val logger: ILogger<T>,
    val mappers: MutableMap<String, MapperHolder<T>>,
) {
    fun postValidate(): Boolean {
        for (mapper in mappers) {
            for (field in mapper.value.fields) {
                val element: IClassModel<T> =
                    (field.value.mapifyElement as? MapifyElementTypeField)?.let {
                        it.element
                    }
                        ?: (field.value.mapifyElement as? MapifyElementTypeGetterSetter)?.let {
                            it.getterSetter.getterElement
                        }
                        ?: throw Exception("unknown kind")

                if (field.value.typeHandleMode == MapifyHolder.TypeHandleMode.UNKNOWN) {
                    logger.error(
                        "the field ${field.key} is not ${Mapifyable::class.java.simpleName} or plain or any other parseable type. ",
                        element.source,
                    )
                }
            }
