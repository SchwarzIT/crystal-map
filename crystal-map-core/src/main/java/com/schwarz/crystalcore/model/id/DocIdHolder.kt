...
class DocIdHolder(
    docId: ISourceDocId,
    val customSegmentSource: MutableList<DocIdSegmentHolder>,
) {
                    ...
                    if (segmentMatcher.find()) {
                        ...
                        segments.add(
                            Segment(
                                ...
                                this.customSegments[
                                    plainSegment
                                        .removeSuffix("(${segmentMatcher.group(1)})")
                                        .removePrefix("this."),
                                ],
                            ),
                        )
    ...
    fun <T> distinctFieldAccessors(model: BaseEntityHolder<T>): List<String> =
        segments
            .map {
                it.fieldsToModelFields(model)
            }.flatten()
            .map { it.accessorSuffix() }
            .distinct()
    ...
    fun <T> companionFunction(
        ...
        val spec =
            FunSpec
                .builder(COMPANION_BUILD_FUNCTION_NAME)
                    ...
                    ?: entityFields
                        .map { (_, fieldHolder) ->
                            ...
                            if (fieldHolder.isNonConvertibleClass) {
                                ...
                                val typeConverter =
                                    typeConvertersByConvertedClass[fieldHolder.fieldType]!!
                                        .instanceClassTypeName.simpleName
    ...
    fun <T> buildExpectedDocId(entity: BaseEntityHolder<T>): FunSpec {
        ...
        if (entity.deprecated?.addDeprecatedFunctions(list.toTypedArray(), spec) == true) {
            ...
            spec.addStatement(
                "return $COMPANION_BUILD_FUNCTION_NAME(${list.joinToString(separator = ",")})",
            )
        }
        return spec.build()
    }
