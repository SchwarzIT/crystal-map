    ...
    fun validate(
        ...
        for (member in mapperElement.source.enclosedElements) {
            if (member.kind == ElementKind.METHOD &&
                member.getAnnotation(Mapify::class.java) != null
            ) {
                ...
                when {
                    isGetter ->
                        getterMap[member.simpleName.toString().substringAfter("get")] =
                            member
                    isSetter ->
                        setterMap[member.simpleName.toString().substringAfter("set")] =
                            member
                    else ->
                        logger.error(
                            "${Mapify::class.java.canonicalName} is only allowed for getters/setters/fields",
                            member,
                        )
                }
            }
        ...
        hashSetOf(
            ...
            *setterMap.keys.minus(getterMap.keys).toTypedArray(),
            ...
            logger.error(
                "${Mapifyable::class.java.simpleName} needs to be applied on getter and setter. ",
                element,
            )
        }
    }
